...
from evomorph.bootstrap.native.bootstrap_compiler import (
    BootstrapCompiler, STDLIB_ASM, LEXER_ASM, TOKEN_BUF
)
...
# Build the compiler .evo source
from evomorph.bootstrap.native.bootstrap_compiler import (
    LOOKUP_ASM, CODEGEN_ASM, PARSER_ASM, MAIN_ASM, INPUT_BUF
)
...
for i in range(min(token_count, 30)):
    ...
    if ttype == 2:
        ...
        print(f"  [{i:3d}] {tname:8s} '{s}'")
    elif ttype == 3:
        print(f"  [{i:3d}] {tname:8s} {tval}")
    elif ttype == 5:
        c_val = chr(tval) if 32 <= tval < 127 else str(tval)
        print(f"  [{i:3d}] {tname:8s} '{c_val}' ({tval})")
    elif ttype == 4:
        s = vm.read_string(tval) if tval > 0 else ""
        print(f"  [{i:3d}] {tname:8s} \"{s}\"")
    else:
